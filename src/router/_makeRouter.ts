...
export function _makeGenericRouter<Model, IPostBody, IPatchBody>(controller:GenericController<Model, IPostBody, IPatchBody>):Router{
    ...
    router
      ...
      .delete("/:userId/:id", async function(req, res, next) {
        try {
          ...
          return res.send(`Deleted Model:${req.params.id}`);
        } catch (err) {
          return next(err);
        }
      })
